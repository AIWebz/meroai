// ---------------------------------------------------------------------------
// Builds the system prompt sent to a live model. Used by every client-side
// AIProvider (LocalModelAIProvider today). Kept in sync with the equivalent
// function in server/mero-ai-backend/src/index.ts (a separate deployable
// package, so not literally shared code, but the same prompt logic).
//
// facts.responseFormat === "json" switches from the normal short, in-
// character chat persona to a structured-data generation mode (used for
// AI-authored website copy) — the two modes want different, sometimes
// conflicting instructions (e.g. "keep replies to 2-5 sentences" would
// actively fight a request for a multi-field JSON object).
// ---------------------------------------------------------------------------

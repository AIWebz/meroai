        ...
        <Card className="flex h-[480px] flex-col p-5 lg:col-span-3">
          <div className="mb-3 flex items-center justify-between">
            <div>
              <div className="flex items-center gap-2">
                ...
                <h2 className="font-display text-[15px] font-semibold text-ink">Mero</h2>
              </div>
              <p className="mt-1 text-[12.5px] text-ink-faint">{ASSISTANT_PERSONA.greeting(company.name)}</p>
            </div>
            <DemoTag />
          </div>
          <div className="min-h-0 flex-1">
            <ChatThread
              ...
              onSend={send}
              sending={sending}
              placeholder="Ask Mero anything about the company…"
              emptyState={
...
function BriefingPanel() {
  ...
  const briefing = useMemo(
    () => generateBriefing({ company, tasks, approvals, activity, opportunities }),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [company, tasks, approvals, activity, opportunities, generatedAt]
  );

...
import { AlertTriangle, CheckCircle2, Cpu, Sparkles, XCircle } from "lucide-react";
...
function AITab() {
  ...
  const isLive = localModelEnabled || localApiKey.trim().length > 0 || backendUrl.trim().length > 0;
  const activeMode = localModelEnabled
    ? `an AI model running in this browser (${LOCAL_MODEL_LABEL})`
    : localApiKey.trim()
      ? "your own key in this browser"
      : backendUrl.trim()
